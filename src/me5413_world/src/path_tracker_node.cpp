...
geometry_msgs::Twist PathTrackerNode::computeControlOutputs(const nav_msgs::Odometry& odom_robot, const geometry_msgs::Pose& pose_goal)
{
  ...
  tf2::fromMsg(odom_robot.pose.pose.orientation, q_robot);  // 当前机器人位置
  tf2::fromMsg(pose_goal.orientation, q_goal);              // 目标点(局部路径的第12个点)
  const tf2::Matrix3x3 m_robot = tf2::Matrix3x3(q_robot);   // 当前机器人的旋转矩阵
  const tf2::Matrix3x3 m_goal = tf2::Matrix3x3(q_goal);     // 目标点位的旋转矩阵

  double roll, pitch, yaw_robot, yaw_goal;
  m_robot.getRPY(roll, pitch, yaw_robot); // 获取当前机器人的欧拉角
  m_goal.getRPY(roll, pitch, yaw_goal);   // 获取目标点位的欧拉角
  ...
  tf2::fromMsg(odom_robot.pose.pose.position, point_robot); // 获取当前机器人位置
  tf2::fromMsg(pose_goal.position, point_goal);             // 获取目标机器人位置
  const tf2::Vector3 V_goal_robot = point_robot - point_goal; // 位置向量相减
