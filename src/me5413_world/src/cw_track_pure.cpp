...
void PathTrackerNode::localPathCallback(const nav_msgs::Path::ConstPtr& path)
{
  ...
  this->pose_world_goal_ = path->poses[20].pose;
...
geometry_msgs::Twist PathTrackerNode::computeControlOutputs(const nav_msgs::Odometry& odom_robot, const geometry_msgs::Pose& pose_goal)
{
  ...
  geometry_msgs::Pose carPose = odom_robot.pose.pose;

  double eta = 0;

  geometry_msgs::Point carPose_pos = carPose.position;
  double x = cos(yaw_robot)*(pose_goal.position.x - carPose_pos.x) + sin(yaw_robot)*(pose_goal.position.y - carPose_pos.y);
  double y = -sin(yaw_robot)*(pose_goal.position.x - carPose_pos.x) + cos(yaw_robot)*(pose_goal.position.y - carPose_pos.y);
  ...
  double lfw = 0.1;
  double Lfw = 1.5;
  double steering_gain = 2.0;
  double speed_incremental = 0.15;

  double steering = atan2((L*sin(eta)),(Lfw/2 + lfw*cos(eta))) * steering_gain;

  velocity = std::min(velocity + speed_incremental, Vcmd);
